...
async function carregar() {
    try {
        ...
        console.error(e);
        alert("Erro ao carregar dados. Verifique a pasta /data.");
    }
}
...
function renderizarTudo() {
    ...
    // Mapa para saber onde cada disciplina está no plano (para o PDF)
    const mapaPlano = {};
    Object.entries(plano).forEach(([semestre, codigos]) => {
        codigos.forEach(c => mapaPlano[c] = semestre);
    });

    // 1. PASSO 1: HISTÓRICO / FLUXOGRAMA
    ...
    Object.keys(semestres).sort((a,b) => a-b).forEach(s => {
        ...
        semestres[s].forEach(d => {
            ...
            const preReqs = d.prerequisitos || [];

            const nomesPre = preReqs.map(cod => {
            ...
            item.innerHTML = `
                ...
                <label for="c-${d.codigo}">
                    ...
                    <small>${d.nome}</small>
                    ${planejadoPara ? `<div class="badge-plano-pdf">Planejada para: ${planejadoPara}</div>` : ''}
...
function renderizarPendencias(ppcKey) {
    ...
    box.innerHTML = '';
    if (!semestreAtivo) {
        box.innerHTML = '<p style="font-size:0.75rem; color:red; padding:10px;">Clique em um semestre no Passo 3.</p>';
        return;
    }
...
function renderizarGrade() {
    ...
    Object.entries(plano).forEach(([sem, codigos]) => {
        ...
        const classeCreditos = totalCreditos > 36 ? 'excesso-creditos' : '';
        coluna.innerHTML = `<h4>${estaAtivo ? '📌 ' : ''}${sem} <span class="badge-creditos ${classeCreditos}">${totalCreditos} CR</span> <span onclick="event.stopPropagation(); removerSemestre('${sem}')">🗑</span></h4>`;
        coluna.appendChild(lista);
...
function addAoPlano(cod) {
    ...
    if (!plano[semestreAtivo].includes(cod)) {
        plano[semestreAtivo].push(cod);
        renderizarTudo();
    }
}
